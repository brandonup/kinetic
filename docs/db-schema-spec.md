...
**Recency columns (migration `20260523000001`, KIN-482, recency-aware retrieval):** `document_date` (raw `knowledge_base_documents.document_date`, nullable) and `document_created_at` (`created_at` cast to `date`) are returned **un-`COALESCE`d** — `retrieval.py` derives `effective_date = COALESCE(document_date, document_created_at)` (post both-tail clamp) and `date_is_estimated = (document_date IS NULL OR implausible)` so the generation layer can distinguish a real publish date from an ingestion-date estimate (see ADR-009, `docs/plans/2026-05-21-recency-aware-retrieval-design.md` § Component B). This is a signature change — the migration `DROP`s all historical overloads (vector/halfvec/text param variants) and recreates with `CREATE` rather than `CREATE OR REPLACE`. All consumers read columns by name, so the two added columns do not break the MCP/Cowork callers.
