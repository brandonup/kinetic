...
**Scope:** This ADR covers Kinetic's hosted MCP server (multi-user, token-authenticated, rate-limited). A separate local MCP server (`kinetic-brain`) was shipped for direct Cowork integration — see `packages/mcp/docs/deployment-guide.md` and `docs/specs/kinetic-brain-plugin-spec.md`.
