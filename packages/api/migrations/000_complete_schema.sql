-- ==========================================================================
-- Kinetic Complete Schema
-- ==========================================================================
-- ONE file. Run this ONCE in Supabase SQL Editor.
-- Safe to re-run: uses IF NOT EXISTS and duplicate-object guards throughout.
--
-- If your database is in a partial/broken state from earlier attempts,
-- uncomment the RESET block below to start clean.
-- ==========================================================================

-- -------------------------------------------------------------------------
-- OPTIONAL RESET — Uncomment these lines ONLY if you want a clean slate.
-- This drops ALL Kinetic tables and types. No data will survive.
-- -------------------------------------------------------------------------
-- DROP TABLE IF EXISTS public.knowledge_base_chunks CASCADE;
-- DROP TABLE IF EXISTS public.knowledge_base_documents CASCADE;
-- DROP TABLE IF EXISTS public.knowledge_base_folders CASCADE;
-- DROP TABLE IF EXISTS public.knowledge_bases CASCADE;
-- DROP TABLE IF EXISTS public.retrieval_debug_logs CASCADE;
-- DROP TABLE IF EXISTS public.mcp_rate_limits CASCADE;
-- DROP TABLE IF EXISTS public.mcp_tokens CASCADE;
-- DROP TABLE IF EXISTS public.memory_proposals CASCADE;
-- DROP TABLE IF EXISTS public.active_memory_entries CASCADE;
-- DROP TABLE IF EXISTS public.framework_trigger_embeddings CASCADE;
...
-- DROP TYPE IF EXISTS api_key_provider CASCADE;
-- DROP TYPE IF EXISTS agent_type CASCADE;
-- DROP TYPE IF EXISTS agent_visibility CASCADE;
-- DROP TYPE IF EXISTS message_role CASCADE;
-- DROP TYPE IF EXISTS framework_confidence CASCADE;
-- DROP TYPE IF EXISTS framework_origin CASCADE;
-- DROP TYPE IF EXISTS proposal_status CASCADE;
-- DROP TYPE IF EXISTS proposal_trigger CASCADE;
-- DROP TYPE IF EXISTS llm_model_category CASCADE;
-- DROP TYPE IF EXISTS retrieval_scope CASCADE;
...
-- 4.1 users
-- Spec: db-schema-spec.md §1. Extends Supabase auth.users via same ID.
...
CREATE TABLE IF NOT EXISTS public.scrape_sources (
  ...
  created_at             timestamptz     NOT NULL DEFAULT now(),
  updated_at             timestamptz     NOT NULL DEFAULT now(),
  CONSTRAINT chk_scrape_sources_source_type CHECK (source_type IN ('substack', 'rss')),
  CONSTRAINT chk_scrape_sources_frequency CHECK (frequency IN ('daily', 'weekly', 'monthly')),
  CONSTRAINT chk_scrape_sources_credential_pair CHECK (
    (credential_ciphertext IS NULL AND credential_nonce IS NULL) OR
    (credential_ciphertext IS NOT NULL AND credential_nonce IS NOT NULL)
  )
);
CREATE INDEX IF NOT EXISTS idx_scrape_sources_poll ON public.scrape_sources (is_active, next_run_at) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_scrape_sources_kb ON public.scrape_sources (knowledge_base_id);
DO $$ BEGIN CREATE TRIGGER trg_scrape_sources_updated_at BEFORE UPDATE ON public.scrape_sources FOR EACH ROW EXECUTE FUNCTION set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
ALTER TABLE public.scrape_sources ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "scrape_sources_select_own" ON public.scrape_sources FOR SELECT USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_sources_insert_own" ON public.scrape_sources FOR INSERT WITH CHECK (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_sources_update_own" ON public.scrape_sources FOR UPDATE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_sources_delete_own" ON public.scrape_sources FOR DELETE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 4.25 scrape_source_posts
CREATE TABLE IF NOT EXISTS public.scrape_source_posts (
  ...
  CONSTRAINT uq_scrape_source_posts_source_external UNIQUE (scrape_source_id, external_id)
);
ALTER TABLE public.scrape_source_posts ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "scrape_source_posts_select_own" ON public.scrape_source_posts FOR SELECT USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_source_posts_insert_own" ON public.scrape_source_posts FOR INSERT WITH CHECK (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_source_posts_update_own" ON public.scrape_source_posts FOR UPDATE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "scrape_source_posts_delete_own" ON public.scrape_source_posts FOR DELETE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- =========================================================================
-- 5. Auth trigger — creates public.users row on Supabase Auth signup
-- =========================================================================
-- Spec: db-schema-spec.md § Auth User Trigger
-- SECURITY DEFINER so the function can write to public.users even though
...
-- NOTE: If Supabase blocks CREATE TRIGGER on auth.users via the SQL editor,
-- create this trigger through the Supabase Dashboard → Database → Triggers.
DO $$ BEGIN
...
-- =========================================================================
-- Done. All 21 tables + auth trigger + RPC functions created with RLS policies and indexes.
-- =========================================================================
