"""
Nate Eval B — Business Consultant persona eval runner.

Measures how well the Nate B. Jones agent performs for Persona B (Business
Consultant) as defined in ``jtbd-nate-agent.md``. Runs 4 binary Pass/Fail
judges against each generation response.
...
Usage:
    ...
    python -m evals.nate_eval_b.eval [--dry-run] [--output-dir <path>]
...
from evals.nate_eval_b.data import (  # noqa: E402
    TEST_CASES,
    NEGATIVE_CONTROLS,
    DRY_RUN_MOCK_RESPONSE,
)
from evals.nate_eval_b.judges import JUDGES, JUDGE_NAMES  # noqa: E402
...
def print_report(metrics: dict, results: list[dict]) -> None:
    ...
    print("\n" + "=" * 65)
    print("NATE EVAL B — BUSINESS CONSULTANT PERSONA RESULTS")
    print("=" * 65)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Nate Eval B — Business Consultant persona eval runner"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: timestamped under evals/nate_eval_b/results/)"
        ),
    )
    parser.add_argument(
        "--conversation-id",
    ...
    print(
        f"Running Nate Eval B — {len(TEST_CASES)} real cases + "
