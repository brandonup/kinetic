"""
Nate Eval C — SMB Owner persona eval runner.

Measures how well the Nate B. Jones agent performs for Persona C (SMB Owner)
as defined in ``jtbd-nate-agent.md``. Runs 4 binary Pass/Fail
...
Usage:
    ...
    python -m evals.nate_eval_c.eval [--dry-run] [--output-dir <path>]
...
from evals.nate_eval_c.data import (  # noqa: E402
    TEST_CASES,
    NEGATIVE_CONTROLS,
    DRY_RUN_MOCK_RESPONSE,
)
from evals.nate_eval_c.judges import JUDGES, JUDGE_NAMES  # noqa: E402
...
def print_report(metrics: dict, results: list[dict]) -> None:
    ...
    print("\n" + "=" * 65)
    print("NATE EVAL C — SMB OWNER PERSONA RESULTS")
    print("=" * 65)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Nate Eval C — SMB Owner persona eval runner"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: timestamped under evals/nate_eval_c/results/)"
        ),
    )
    parser.add_argument(
        "--conversation-id",
    ...
    print(
        f"Running Nate Eval C — {len(TEST_CASES)} real cases + "
